...
class ServerApp:
    ...
    def _ui(self):
        st = ttk.Style()
        st.theme_use("clam")
        st.configure("T", font=("Segoe UI", 18, "bold"), foreground="#e94560", background="#1a1a2e")
        st.configure("I", font=("Segoe UI", 11), foreground="#eee", background="#16213e")
        st.configure("C", background="#16213e")
        st.configure("B", font=("Segoe UI", 11, "bold"))

        ttk.Label(self.root, text="⚡ RDP Server", style="T").pack(pady=15)
        c = ttk.Frame(self.root, style="C", padding=15)
        c.pack(fill="x", padx=20, pady=5)
        ttk.Label(c, text=f"🌐 IP: {get_ip()}", style="I").pack(anchor="w")
        
        pf = ttk.Frame(c, style="C")
        pf.pack(anchor="w", fill="x", pady=2)
        ttk.Label(pf, text="🔌 Port: ", style="I").pack(side="left")
        ...
        ttk.Label(c, textvariable=self.stat_var, style="I").pack(anchor="w", pady=5)
        self.cli_var = tk.StringVar(value="")
        ttk.Label(c, textvariable=self.cli_var, style="I").pack(anchor="w")

        bf = ttk.Frame(self.root, style="C", padding=5)
        bf.pack(fill="x", padx=20)
        self.start_btn = ttk.Button(bf, text="▶ Start", style="B", command=self._start)
        self.start_btn.pack(side="left", padx=2, expand=True, fill="x")
        self.stop_btn = ttk.Button(bf, text="⏹ Stop", style="B", command=self._stop, state="disabled")
        self.stop_btn.pack(side="left", padx=2, expand=True, fill="x")
        self.file_btn = ttk.Button(bf, text="📁 File", style="B", command=self._send_file_dlg, state="disabled")
